...
# Create a dummy package structure to allow installation
RUN mkdir -p src/qwen_code_api && touch src/qwen_code_api/__init__.py
...
# Reinstall the package with the actual source
RUN --mount=type=cache,target=/root/.cache/uv \
    uv pip install -e .
